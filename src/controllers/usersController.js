...
export const createUser = async (req, res) => {
	...
	try {
		...
		createTokenForNewUser(createdUser, res);
...
const createTokenForNewUser = async (user, res) => {
...
export const updateUser = async (req, res) => {
	...
	try {
		...
		const resUser = await User.findByIdAndUpdate(userId, user, {
			new: true,
		});
		res.status(200).json({
			user: resUser,
			message: `${res.username} updated, successfully`,
		});
	} catch (error) {
